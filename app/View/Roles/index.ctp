                            ...
                            <tbody>
                                <?php foreach ($roles as $role): ?>
                                    <tr>
                                        ...
                                        <td><span class="label" style="background:<?php echo $role['Role']['color_code'];?>"><?php echo h($role['Role']['title']); ?></span></td>
                                        <td><?php echo h($role['Role']['description']); ?></td>
                                        <td><?php echo $status[$role['Role']['status']]; ?></td>
                                        <td class="actions">
                                            <?php echo $this->Html->link(__('<i class="icon-eye" title="View"></i>'), array('action' => 'view', $role['Role']['id']), array('escape' => false)); ?>
                                            <?php echo $this->Html->link(__('<i class="icon-pencil7 text-success" title="Edit"></i>'), array('action' => 'edit', $role['Role']['id']), array('escape' => false)); ?>
                                            <?php echo $this->Form->postLink(__('<i class="icon-trash text-danger" title="Delete"></i>'), array('action' => 'delete', $role['Role']['id']), array('confirm' => __('Are you sure you want to delete # %s?', $role['Role']['id']), 'escape' => false)); ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
